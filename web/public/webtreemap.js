...
(function () {
  ...
  function focus(tree) {
    ...
    while (root.parent) {
      ...
      for (var i = 0, sibling; (sibling = root.children[i]); ++i) {
        if (sibling.dom) sibling.dom.style.zIndex = 0;
      }
    }
  ...
  function makeDom(tree, level) {
    ...
    if (tree.data["$symbol"]) {
      dom.className +=
        " webtreemap-symbol-" + tree.data["$symbol"].replace(" ", "_");
    }
    if (tree.data["$dominant_symbol"]) {
      dom.className +=
        " webtreemap-symbol-" + tree.data["$dominant_symbol"].replace(" ", "_");
  ...
  function selectSpan(nodes, space, start) {
    ...
    for (var end = start; (node = nodes[end]); ++end) {
      ...
      if (size < rmin) rmin = size;
      if (size > rmax) rmax = size;
      rsum += size;
  ...
  function layout(tree, level, width, height) {
    ...
    for (var start = 0, child; (child = tree.children[start]); ++start) {
      ...
      if (ysplit) space = (y2 - y1) * pixels_to_units;
      else space = (x2 - x1) * pixels_to_units;
      ...
      if (ysplit) x1 += Math.round(rsum / space / pixels_to_units);
